...
describe("OtterAllowListAuthentication", () => {
  ...
  beforeEach(async () => {
    const OtterAllowListAuthentication = await ethers.getContractFactory(
      "OtterAllowListAuthenticationTestInterface",
      deployer,
    );
    ...
    authenticator = await OtterAllowListAuthentication.deploy(owner.address);
  ...
  describe("addSolver(address)", () => {
    ...
    it("should not allow owner to add solver", async () => {
      ...
      ).to.be.revertedWith("Otter: caller not manager");
    });
