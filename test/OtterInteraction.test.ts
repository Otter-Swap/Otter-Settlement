...
describe("OtterInteraction", () => {
  const [deployer, trader] = waffle.provider.getWallets();
  let interactions: Contract;

  beforeEach(async () => {
    const OtterInteraction = await ethers.getContractFactory(
      "OtterInteractionTestInterface",
    );
    interactions = await OtterInteraction.deploy();
  });
