...
import { InteractionLike, InteractionStage, TradeSimulator } from "../src/";
...
describe("OtterTradeSimulator", () => {
  ...
  beforeEach(async () => {
    ...
    const OtterTradeSimulator = await ethers.getContractFactory(
      "OtterTradeSimulator",
      deployer,
    );
    tradeSimulator = new TradeSimulator(
      settlement,
      await OtterTradeSimulator.deploy(),
    );
