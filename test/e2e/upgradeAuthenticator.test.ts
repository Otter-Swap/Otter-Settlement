...
describe("E2E: Upgrade Authenticator", () => {
  ...
  it("should upgrade authenticator", async () => {
    const OtterAllowListAuthenticationV2 = await ethers.getContractFactory(
      "OtterAllowListAuthenticationV2",
      deployer,
    );
    // Note that, before the upgrade this is actually the old instance
    const authenticatorV2 = OtterAllowListAuthenticationV2.attach(
