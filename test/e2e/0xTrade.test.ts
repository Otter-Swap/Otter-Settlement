...
describe("E2E: Can settle a 0x trade", () => {
  ...
  function generateSettlementSolution(): {
    ...
    const zeroExTakerAmount = otterOrder.buyAmount.mul(zeroExGnoPrice);

    const otterGnoPrice = 120;
    const clearingPrices = {
      [owl.address]: 1,
      [gno.address]: otterGnoPrice,
    };

    const otterOwlSurplus = otterOrder.sellAmount.sub(
      otterOrder.buyAmount.mul(otterGnoPrice),
    );
    const zeroExOwlSurplus = otterOrder.buyAmount.mul(
      otterGnoPrice - zeroExGnoPrice,
    );
  ...
  describe("0x Protocol v2", () => {
    it("should settle an EOA trade with a 0x trade", async () => {
      ...
      await encoder.signEncodeTrade(otterOrder, trader, SigningScheme.EIP712);
      ...
      expect(await owl.balanceOf(trader.address)).to.deep.equal(otterOwlSurplus);
      // NOTE: The exchange keeps the surplus from the 0x order.
      expect(await owl.balanceOf(settlement.address)).to.deep.equal(
        zeroExOwlSurplus.add(otterOrder.feeAmount),
      );
    });
  });
});
