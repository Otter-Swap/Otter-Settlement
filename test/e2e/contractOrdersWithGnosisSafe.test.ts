...
describe("E2E: Order From A Gnosis Safe", () => {
  ...
  it("should settle matching orders", async () => {
    ...
    const otterMessage = hashOrder(domainSeparator, order);
    const safeAsFallback = gnosisSafeManager.signingFallback.attach(
      safe.address,
    );
    // Note: threshold is 2, any two owners should suffice.
    const signature = await fallbackSign(safeAsFallback, otterMessage, [
      safeOwners[4],
      safeOwners[2],
    ]);
    ...
    expect(
      await safeAsFallback.callStatic[isValidSignature](otterMessage, signature),
