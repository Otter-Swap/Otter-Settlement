...
describe("OtterSigning", () => {
  ...
  beforeEach(async () => {
    const OtterSigning = await ethers.getContractFactory(
      "OtterSigningTestInterface",
    );

    signing = await OtterSigning.deploy();
  ...
  describe("domainSeparator", () => {
    ...
    it("should have a different replay protection for each deployment", async () => {
      ...
      const signing2 = await OtterSigning.deploy();
  ...
  describe("recoverOrderFromTrade", () => {
    ...
    describe("uid uniqueness", () => {
      it("invalid EVM transaction encoding does not change order hash", async () => {
        ...
        // a valid transaction.
        // Computing Otter's orderUid requires copying 32-byte-encoded addresses
