...
import "./OtterVaultRelayer.sol";
import "./interfaces/OtterAuthentication.sol";
...
/// @title Otter Settlement Contract
/// @author Otter Developers
contract OtterSettlement is Signing, ReentrancyGuard, StorageAccessible {
    using Order for bytes;
    using OtterTransfer for IVault;
    ...
    /// modifier below.
    OtterAuthentication public immutable authenticator;
    ...
    OtterVaultRelayer public immutable vaultRelayer;
    ...
    constructor(OtterAuthentication authenticator_, IVault vault_) {
        authenticator = authenticator_;
        vault = vault_;
        vaultRelayer = new OtterVaultRelayer(vault_);
    }
    ...
    modifier onlySolver() {
        require(authenticator.isSolver(msg.sender), "Otter: not a solver");
        _;
    }
    ...
    modifier onlyInteraction() {
        require(address(this) == msg.sender, "Otter: not an interaction");
        _;
    }

    /// @dev Settle the specified orders at a clearing price. Note that it is
    /// the responsibility of the caller to ensure that all Otter invariants are
    ...
    function settle(
        ...
        OtterInteraction.Data[][3] calldata interactions
    ...
    function swap(
        ...
        require(filledAmount[orderUid] == 0, "Otter: order filled");
        if (order.kind == Order.KIND_SELL) {
            require(
                executedSellAmount == order.sellAmount,
                "Otter: sell amount not respected"
            );
            filledAmount[orderUid] = order.sellAmount;
        } else {
            require(
                executedBuyAmount == order.buyAmount,
                "Otter: buy amount not respected"
            );
    ...
    function invalidateOrder(bytes calldata orderUid) external {
        ...
        require(owner == msg.sender, "Otter: caller does not own order");
        ...
        )
    {
        ...
        inTransfers = new OtterTransfer.Data[](trades.length);
        outTransfers = new OtterTransfer.Data[](trades.length);

        for (uint256 i = 0; i < trades.length; i++) {
            OtterTrade.Data calldata trade = trades[i];
    ...
    function computeTradeExecution(
        ...
        require(order.validTo >= block.timestamp, "Otter: order expired");
        ...
        require(
            ...
            "Otter: limit price not respected"
        );
    ...
    function executeInteractions(OtterInteraction.Data[] calldata interactions)
        internal
    {
        for (uint256 i; i < interactions.length; i++) {
            OtterInteraction.Data calldata interaction = interactions[i];
            ...
            emit Interaction(
                interaction.target,
                interaction.value,
                OtterInteraction.selector(interaction)
            );
        }
    }
    ...
    function freeOrderStorage(
        ...
        for (uint256 i = 0; i < orderUids.length; i++) {
            ...
            // solhint-disable-next-line not-rely-on-time
            require(validTo < block.timestamp, "Otter: order still valid");
