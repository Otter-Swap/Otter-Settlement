...
/// @title Otter OtterTrade Simulator
/// @author Otter Developers
contract OtterTradeSimulator {
    ...
    struct Context {
        uint256 gasCounter;
        OtterTransfer.Data[] inTransfers;
        OtterTransfer.Data[] outTransfers;
    ...
    function simulateTrade(
        ...
        OtterSettlement self = OtterSettlement(payable(address(this)));
        ...
        private
    {
        for (uint256 i; i < interactions.length; i++) {
            OtterInteraction.execute(interactions[i]);
        }
    }
        ...
        returns (Context memory context)
    {
        ...
        address actualReceiver = trade.receiver ==
            Order.RECEIVER_SAME_AS_OWNER
            ? trade.owner
            : trade.receiver;

        context.inTransfers = new OtterTransfer.Data[](1);
        {
            OtterTransfer.Data memory inTransfer = context.inTransfers[0];
        ...
        context.outTransfers = new OtterTransfer.Data[](1);
        {
            OtterTransfer.Data memory outTransfer = context.outTransfers[0];
