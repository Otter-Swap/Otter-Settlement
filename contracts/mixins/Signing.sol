...
import "../interfaces/OtterEIP1271.sol";
import "../libraries/Order.sol";
import "../libraries/OtterTrade.sol";

/// @title Otter Signing Library.
...
abstract contract Signing {
    ...
    bytes32 private constant DOMAIN_NAME = keccak256("Otter Protocol");
    ...
    uint256 private constant PRE_SIGNED =
        uint256(keccak256("OtterSigning.Scheme.PreSign"));
    ...
    function setPreSignature(bytes calldata orderUid, bool signed) external {
        ...
        require(owner == msg.sender, "Otter: cannot presign order");
        if (signed) {
    ...
    function recoverOrderFromTrade(
        ...
        Scheme signingScheme = OtterTrade.extractOrder(trade, tokens, order);
        ...
        returns (address signer)
    {
        require(
            ...
            "Otter: malformed ecdsa signature"
        );
        ...
        require(signer != address(0), "Otter: invalid ecdsa signature");
    }
    ...
    function recoverEip1271Signer(
        ...
        require(
            EIP1271Verifier(owner).isValidSignature(orderDigest, signature) ==
                OtterEIP1271.MAGICVALUE,
            "Otter: invalid eip1271 signature"
        );
    }
    ...
    function recoverPreSigner(
        ...
        require(encodedSignature.length == 20, "Otter: malformed presignature");
        ...
        bytes memory orderUid = new bytes(Order.UID_LENGTH);
        orderUid.packOrderUidParams(orderDigest, owner, validTo);

        require(
            preSignature[orderUid] == PRE_SIGNED,
            "Otter: order not presigned"
        );
    }
}
