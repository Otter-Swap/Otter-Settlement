...
import "./Order.sol";
import "./OtterSafeERC20.sol";

/// @title Otter Transfers
/// @author Otter Developers
library OtterTransfer {
    ...
    function fastTransferFromAccount(
        ...
        require(
            ...
            "Otter: cannot transfer native ETH"
        );
        ...
        internal
    {
        ...
        for (uint256 i = 0; i < transfers.length; i++) {
            ...
            if (address(transfer.token) == BUY_ETH_ADDRESS) {
                require(
                    transfer.balance != Order.BALANCE_INTERNAL,
                    "Otter: unsupported internal ETH"
                );
                payable(transfer.account).transfer(transfer.amount);
            } else if (transfer.balance == Order.BALANCE_ERC20) {
