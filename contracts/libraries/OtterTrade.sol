...
/// @title Otter OtterTrade Library.
...
library OtterTrade {
    ...
    ) internal pure returns (Signing.Scheme signingScheme) {
    ...
    ///
    /// OtterTrade flags are used to tightly encode information on how to decode
        ...
        )
    {
        ...
        if (flags & 0x08 == 0) {
            ...
            sellTokenBalance = Order.BALANCE_EXTERNAL;
        } else {
            sellTokenBalance = Order.BALANCE_INTERNAL;
        }
        if (flags & 0x10 == 0) {
            buyTokenBalance = Order.BALANCE_ERC20;
        } else {
            buyTokenBalance = Order.BALANCE_INTERNAL;
        }
        ...
        signingScheme = Signing.Scheme(flags >> 5);
    }
}
