...
import { domain, OrderKind, SigningScheme, signOrder } from "../src";
import {
  Api,
  Environment,
  SellAmountAfterFee,
...
const APP_DATA = keccak("Otter place order script");
...
async function placeOrder(
  ...
  const [[signer], settlement, chainId] = await Promise.all([
    ...
    getDeployedContract("OtterSettlement", hre),
    hre.getChainId(),
  ]);
...
const setupPlaceOrderTask: () => void = () => {
  task(
    "place-order",
    "Places a limit order on Otter according to the price estimation endpoint with 30 minute validity and 0 slippage",
  )
