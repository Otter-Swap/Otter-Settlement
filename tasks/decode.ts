...
async function calldataFromUserInput(
  ...
  if (txhash !== undefined) {
    ...
    if (deployment === null || tx.to !== deployment.address) {
      console.log(
        `Warning: the input transaction hash does not point to an interaction with the current deployment of Otter settlement contract on ${network.name}.`,
      );
...
const setupDecodeTask: () => void = () => {
  task("decode", "Decodes Otter settlement calldata.")
    .addOptionalParam(
      "txhash",
    ...
    .setAction(async ({ txhash }, hre) => {
      ...
      const OtterSettlement = await artifacts.readArtifact("OtterSettlement");
      const settlementInterface = new utils.Interface(OtterSettlement.abi);
