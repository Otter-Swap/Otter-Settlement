...
import { BUY_ETH_ADDRESS } from "../src";
import { Api, Environment } from "../src/api";
...
export interface State {
  ...
  /**
   * All tokens ever traded on Otter. Stored in the state to avoid stressing the
 ...
 * withdrawn amount up for trade in Otter in exchange for a target token. The
      ...
      async (
        ...
        try {
          [authenticator, settlementDeployment, [solver]] = await Promise.all([
            getDeployedContract("OtterAllowListAuthentication", hre),
            hre.deployments.get("OtterSettlement"),
